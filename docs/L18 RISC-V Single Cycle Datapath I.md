...
- **PC + 4**：程序计数器递增4，指向下一条指令。
- **IMEM**：根据PC获取当前指令，并将其分解以索引寄存器文件。
- **RegFile**：从寄存器中读取`rs1`和`rs2`的值，送入ALU进行计算。
- **ALU**：根据指令执行相应的算术或逻辑操作，将结果写回寄存器文件。
...
| funct7  | funct3 | 指令 | 操作码  |
| ------- | ------ | ---- | ------- |
| 0000000 | 000    | add  | 0110011 |
| 0100000 | 000    | sub  | 0110011 |
| 0000000 | 001    | sll  | 0110011 |
| 0000000 | 010    | slt  | 0110011 |
| 0000000 | 011    | sltu | 0110011 |
| 0000000 | 100    | xor  | 0110011 |
| 0000000 | 101    | srl  | 0110011 |
| 0100000 | 101    | sra  | 0110011 |
| 0000000 | 110    | or   | 0110011 |
| 0000000 | 111    | and  | 0110011 |
...
1. **程序计数器（PC）**：PC 增加 4 以指向下一条指令。
2. **指令存储器（IMEM）**：IMEM 根据 PC 提供当前指令，并将指令分解为 `rs1`、`rd` 和 `imm`。
3. **寄存器文件（RegFile）**：
   - 读取 `rs1` 的值，提供给 ALU 的 A 输入端。
   - `rs2` 的值仍然输入到 RegFile，但由于 Bsel = 1，`rs2` 的数据线将被忽略。
4. **立即数生成模块（Imm. Gen.）**：
   - 从指令的 `imm` 字段生成 32 位立即数 `imm`，并提供给多路复用器的一个输入。
5. **多路复用器（MUX）**：
   - 控制线 Bsel = 1 选择生成的立即数 `imm` 作为 ALU 的 B 输入端。
6. **算术逻辑单元（ALU）**：
   - 执行加法操作，将 `rs1` 和 `imm` 相加，并将结果写入目标寄存器 `rd`。
7. **控制逻辑**：
   - 控制信号 Bsel 和 ALUSel 由控制逻辑生成，用于选择 ALU 操作的输入和类型。
